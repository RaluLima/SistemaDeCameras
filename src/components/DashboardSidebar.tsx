...
const adminLinks = [
  ...
  { href: "/admin/live", label: "Ao Vivo", icon: "📡" },
  ...
  { href: "/admin/servers", label: "Servidores", icon: "🖥️" },
];

const userLinks = [
  { href: "/user/cameras", label: "Minhas Câmeras", icon: "📷" },
  { href: "/user/live", label: "Ao Vivo", icon: "📡" },
  { href: "/user/recordings", label: "Minhas Gravações", icon: "🎬" },
  { href: "/user/alerts", label: "Meus Alertas", icon: "🔔" },
  { href: "/user/groups", label: "Meus Grupos", icon: "📁" },
];

function NavItems({ role, isPaying, links, pathname, theme, toggle, mobileOpen, setMobileOpen }: {
  role: string; isPaying: boolean; links: typeof adminLinks; pathname: string;
  theme: string; toggle: () => void; mobileOpen: boolean; setMobileOpen: (v: boolean) => void;
}) {
